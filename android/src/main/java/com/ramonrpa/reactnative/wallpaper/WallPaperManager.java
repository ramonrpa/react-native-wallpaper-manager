package com.ramonrpa.reactnative.wallpaper;
...
public class WallPaperManager extends ReactContextBaseJavaModule {
    ...
    private SimpleTarget<byte[]> getSimpleTarget(final String source, final String screen){
        ...
        return new SimpleTarget<byte[]>(metrics.widthPixels, metrics.heightPixels){
            @Override
            public void onResourceReady(byte[] resource, GlideAnimation<? super byte[]> glideAnimation) {
                ...
                try
                {
                    if (VERSION.SDK_INT >= 24) {
                        if (screen.equalsIgnoreCase("both")){
                    ...
                    sendMessage("success","Set Wallpaper Success",screen);
                }
                catch (Exception e)
                {
                    sendMessage("error","Exception in SimpleTarget：" + e.getMessage(),source);
